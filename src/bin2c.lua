...
Evan Wies added the ability to specify the lua_State variable name
and nargs and nresults
]=]
...
local status = arg[2]
local L = arg[3] or 'L'
local nargs = arg[4] or 0
local nresults = arg[5] or 0
...
/* #include'ing this file in a C program is equivalent to calling
  if (%sluaL_loadfile(%s,%q)%s==0) %slua_pcall(%s, %s, %s, 0); 
*/
/* %s */
static const unsigned char B1[]={
 ...
 if (%sluaL_loadbuffer(%s,(const char*)B1,sizeof(B1),%q)%s==0) %slua_pcall(%s, %s, %s, 0);
}
]=])